...
**Concurrency:** `ConnectionManager` uses a **`sync.RWMutex`** (`mu`). Writers take **`Lock`** for mutations (registering/removing pools, `activeConn`, tunnel metadata); readers take **`RLock`** in **`GetActive`**, **`GetActiveDB`**, **`List`**, **`GetServerType`**, etc. The add path holds the mutex only for short check-and-register critical sections; **`Ping()`**, SSH tunnel setup, and server-type detection run **without** holding `mu` so concurrent readers are not stalled for network I/O. **`getDB()`** resolves the active pool under **`RLock`** via **`GetActiveDB`**. Do not call other lock-taking `ConnectionManager` methods while already holding **`Lock`**.

## 3. Data Flow (`add_connection`)
1. The user/LLM calls `add_connection` with `name`, `dsn`, and an optional `description`.
2. The server validates that the name is unique and not already registered on the `ConnectionManager` (atomic check-and-add under **`Lock`** for the insert path).
